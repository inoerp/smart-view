...
//#path_by_module a, #pagination .page_nos a, .pagination_page .page_nos a
 $('body').on('click', '#top-path-menu-ul a, #path_by_module a, .search_result a,#erp_form_area a.ajax-link,.page_nos.pagination a , .pagination_page .page_nos a, #header_top .menu a, #sys_menu_left_vertical .menu a,#search_result .action a,  #new_page_button', function (e) {
  e.preventDefault();
...
//open new period
 ...
 $('body').on('click', '.remove-filter', function () {
  ...
  $('#searchForm').find('ul.search_form').find(field_name).remove();
...
//   console.log(no_of_item);
 ...
 $('body').on('click', '#save_ora_login', function (e) {
  e.preventDefault();
  $.when(save_dataInSession({
   data_name: 'ORA_USER_NAME',
   data_value: $('#ORA_USER_NAME').val(),
   over_write: false
  })).then(function () {
   $.when(save_dataInSession({
    data_name: 'ORA_PASSWORD',
    data_value: $('#ORA_PASSWORD').val(),
    over_write: false
   })).then(function () {
     alert('Oracle username & password saved');  
   });
  });
 });
