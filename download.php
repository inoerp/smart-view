...
if (!empty($_POST) && !empty($_POST['program_name'])) {
 if ((!empty($_POST['class_name'])) && (!empty($_POST['program_name']))) {
  ...
  $download_format = !empty($_POST['download_format'][0]) ? $_POST['download_format'][0] : 'excel_format';
 }
} else {
 $str_var = $_POST["data"];
 $array_var = unserialize(base64_decode($str_var));
 if(!empty($_POST['download_format'])){
  $download_format = is_array($_POST['download_format']) ? $_POST['download_format'][0] : 'excel_format';
 }else{
  $download_format =  'excel_format';
 }
